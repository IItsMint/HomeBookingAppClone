...
import com.example.HomeBookingApp_back.user.application.UserException;
...
public class LandlordResource {
            ...
            ) throws IOException
    {
       ...
       if(!violations.isEmpty()){
           String violationsJoined = violations.stream().map(violation -> violation.getPropertyPath()+ " "
    ...
    private static Function<MultipartFile, PictureDTO> mapMultipartFileToPictureDTO() {
        return multipartFile -> {
            try {
                ...
                throw new UserException(String.format("Could not parse multipart file: %s", multipartFile.getOriginalFilename()));
            }
        };
    }

    @GetMapping(value = "/get-all")
    @PreAuthorize("hasAnyRole('"+ SecurityUtils.ROLE_LANDLORD+ "')") //To restrict the access.
