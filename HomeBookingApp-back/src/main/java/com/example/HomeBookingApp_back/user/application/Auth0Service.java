package com.example.HomeBookingApp_back.user.application;
...
public class Auth0Service {
    ...
    private void assignRoleById(String accessToken, String email, UUID publicId, String roleIdToAdd) throws Auth0Exception {
        ...
        User user = auth0userByEmail.getBody().stream().findFirst().orElseThrow(() ->
                new UserException(String.format("Could not find user with public id %s", publicId)));
    ...
    public void  addLandlordRoleToUser(ReadUserDTO readUserDTO) {
        if (readUserDTO.authorities().stream().noneMatch(role -> role.equals(SecurityUtils.ROLE_LANDLORD))) {
            try {
                ...
                throw new UserException(String.format("Could not possible to assign %s to %s", roleLandlordId, readUserDTO.publicId()));
            }
        }
    }
