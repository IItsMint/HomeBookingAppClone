...
//we need to listen the signal, so that we realize the changes in listing.
  private listenToListing(){

    effect(() => {
      const listing = this.listing();
      this.countryService.getCountryByCode(listing.location).subscribe({next:country =>{
  ...
  private listenToCardMode(){
    effect(() => {
      ...
      //since we have two kind of variable, booking listing and card listing so we wanna know which one.
